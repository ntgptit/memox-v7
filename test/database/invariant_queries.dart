/// The 22 invariant queries this suite executes, copied verbatim from
/// `data-model.md`.
///
...
const Map<String, String> invariantQueries = <String, String>{
  ...
  'Q31': // Active card inside a deleted deck (BR-182, BR-184)
      'SELECT c.id FROM cards c '
      'JOIN decks d ON d.id = c.deck_id '
      'WHERE c.delete_batch_id IS NULL AND d.delete_batch_id IS NOT NULL',

  'Q32': // Active deck under a deleted deck (BR-182, BR-184)
      'SELECT d.id FROM decks d '
      'JOIN decks p ON p.id = d.parent_deck_id '
      'WHERE d.delete_batch_id IS NULL AND p.delete_batch_id IS NOT NULL',

  'Q33': // A batch that owns no rows at all (BR-191)
  ...
  'Q34': // A tombstone deleted AFTER its already-deleted ancestor (BR-184)
      'SELECT d.id FROM decks d '
      'JOIN decks p ON p.id = d.parent_deck_id '
  ...
  'Q35': // A batch not pointing at an item root carrying it (BR-182)
      'SELECT b.id FROM delete_batches b '
