...
/// The auditable half of BR-183: **every statement that reads `cards` or
/// `decks` either excludes tombstones or is on this list with a reason.**
///
/// BR-183 is the only rule in the document that constrains a *shape* rather
...
void main() {
  ...
  /// tombstones. Every entry is a decision recorded in AD-21, not a backlog.
  const exempt = <String, String>{
    'updateSubtreeRootDeck':
        'AD-21 — a tombstone inside a moved subtree must follow it to the new '
        'root, or invariant 6 fires and a later restore hands back a branch '
        'naming a root it does not belong to.',
    'resetTreeStudyStates':
        'AD-21 — a card in Trash is still in the tree, and invariant 9 compares '
  ...
  test('every active statement reading cards or decks excludes tombstones', () {
    ...
    expect(
      ...
      reason:
          ...
          '(BR-183). Add the predicate, or add the statement to `exempt` with '
