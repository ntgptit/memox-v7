...
void main() {
  ...
  invariantTest(
    'Q31',
    'a recall direction outside where BR-182 allows one',
    // The seed's session is `reviewing`/`self_assess` but its root runs
    // `eight_box`, which is the combination BR-182 names first: the algorithm
  ...
  invariantTest(
    ...
    // direction is exactly the row BR-182 says cannot exist — and it is the one
  ...
  invariantTest(
    'Q32',
    "a turn's direction disagrees with the queue row it came from (BR-185)",
