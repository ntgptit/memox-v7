...
/// The v7 → v8 upgrade: `app_settings.theme_mode` and `app_settings.language`
/// (BR-186, BR-187).
///
...
/// untouched.
///
/// Seeded through **raw SQL against the v7 schema**, because the claim is about
...
void main() {
  ...
  /// seeds the row, so a v7 database already has one and an `INSERT` alone
  ...
  group('v7 → v8', () {
    test(
      'a v7 database upgrades and validates against the v8 declaration',
      () async {
    ...
    test('both new columns default to system, which is what v7 did', () async {
      // Not a placeholder: every build before v8 followed the platform for
    ...
    test('nothing outside app_settings is touched', () async {
      ...
      // future edit adding a second statement to `_upgradeToV8` without saying
