...
void main() {
  for (final RoleBinding binding in roleBindings) {
    test('${binding.component} · ${binding.slot}', () {
      ...
      // the source said which one the slot read. Since 2026-09-17 (GC-3) a
      // row may *require* a semantic token — the brand's ink for a text slot
      // — and then that token, and only that one, is not a substitute.
...
class RoleBinding {
  ...
  /// text slot (GC-3, 2026-09-17). Empty for every role-only row.
