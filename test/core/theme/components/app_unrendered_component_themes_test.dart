...
void main() {
  ...
  group('tab bar', () {
    test('the selected label reads on the page it sits on', () {
      // A tab's label sits on the page, not on a container fill.
      // `_TabBarDefaultsM3` inks it `primary`, but v3's light `primary` fails
      // 4.5:1 as bare text (GC-3, 2026-09-17), so the label takes
      // `AppSemanticColors.accentInk` instead; the indicator below keeps the
      // canonical fill role. This asks whether the label is readable where it
      // actually lands.
      for (final entry in themes.entries) {
        final t = entry.value;
        final semantic = t.extension<AppSemanticColors>()!;

        expect(t.tabBarTheme.labelColor, semantic.accentInk);
        expect(
