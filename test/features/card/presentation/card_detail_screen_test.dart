...
/// when it cannot show anything (UC-19, M4.15 W3 faces 1, 2, 7 and 8).
///
...
void main() {
  ...
  group('current state (BR-240, AD-08)', () {
    ...
    testWidgets('no aggregate is shown anywhere (BR-243)', (tester) async {
      final repository = FakeCardDetailRepository()
        ..seededDetail = fakeCardDetail()
  ...
  group('failure faces (BR-245, UC-19 E1/E3)', () {
    ...
    testWidgets('a card that disappears while it is open loses the Edit '
      ...
      // not-found face pointing at a card that is gone (M4.15 W3 face 8).
      expect(find.text('Card not found'), findsOneWidget);
      expect(find.byIcon(Icons.edit_outlined), findsNothing);
    });
  ...
  group('editing is an explicit action (BR-246)', () {
