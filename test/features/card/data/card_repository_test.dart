...
void main() {
  ...
  group('updateCard and deleteCard', () {
    ...
    test('deleting a card keeps its state and history (BR-185)', () async {
      final seeded = await seedCard();
      await insertSession(
        h.db,
        id: 'session-1',
      ...
      // **The row stays, and that is the whole of BR-185.** It used to be
      ...
      // visibility: the card leaves every active read (BR-183) while the three
    ...
    test('deleting one of several cards leaves the type alone', () async {
      ...
      // tombstone too — the number that matters to BR-186 is the active one,
