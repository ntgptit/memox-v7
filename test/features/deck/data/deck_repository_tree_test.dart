...
void main() {
  ...
  group('deleteDeck and getDeletionImpact', () {
    ...
    test('deleting a root hides the entire tree (BR-03, BR-256)', () async {
      final tree = await h.seedTree();
      await h.cardRepository.createCard(
        deckId: tree.leaf.id,
        front: cardText('f'),
      ...
      // it" is now marking rather than deleting (BR-256), so this asserts the
      // two halves that replaced the old row counts: every deck and card is
      // invisible, and every row a restore needs is still there (BR-259).
      ...
      // (BR-259, BR-86): its turns stay in history.
    ...
    test('deleting a branch keeps the rest of the tree', () async {
      ...
      // batch (BR-258), and the card under them went with it.
