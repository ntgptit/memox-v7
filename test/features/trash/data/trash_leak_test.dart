...
/// **The semantic half of BR-183.** `query_inventory_test.dart` proves every
...
void main() {
  ...
  test('the deletion impact counts only what is still there', () async {
    ...
    // may promise to take, because it is not going anywhere (BR-184).
  ...
  test('a deck in Trash reads as gone to every write path', () async {
    ...
    // tombstone into `NotFoundFailure` — which is what BR-183 means by "hidden"
    // on the write side.
    await expectLater(
