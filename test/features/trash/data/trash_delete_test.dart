...
/// The soft-delete decision table (BR-182…BR-186), against real SQLite.
void main() {
  ...
  group('one card', () {
    test('the row stays, invisible, in a batch of its own', () async {
      ...
      // (BR-185), so none of them may be cascaded away by a delete.
      expect(await h.countAll('cards'), 2);
      expect(await h.countAll('card_study_states'), 2);
      // Hidden: the deck reads one card, not two (BR-183).
    ...
    test('a bulk delete makes one batch per card (BR-182)', () async {
    ...
    test('the deck goes back to unset once nothing active is left', () async {
      ...
      await h.cardRepository.deleteCard(tree.cardIds.first);
      // One still visible, so the type stands (BR-186).
      expect(await h.contentTypeOf(tree.leaf.id), 'card');
  ...
  group('one deck', () {
    test('the whole active subtree joins one batch (BR-184)', () async {
      final tree = await h.seedTree(cardCount: 2);
    ...
    test('a descendant already in Trash keeps its older batch', () async {
      ...
      // **The card does not join the deck's batch** (BR-184). It was deleted
