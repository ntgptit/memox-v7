...
/// Restore and Undo (BR-187, BR-188, BR-189), against real SQLite.
void main() {
  ...
  group('a card', () {
    test('goes back with its id, state and history intact', () async {
      ...
      // Nothing was recreated, so nothing could be recreated wrongly (BR-188).
  ...
  group('a deck subtree', () {
    test('revives only its own batch (BR-184, BR-188)', () async {
    ...
    test('rewrites root_deck_id for tombstones inside it too', () async {
      // The half of BR-188 that only invariant 6 would catch: a branch deleted
    ...
    test('a root deck restores only to the top level (BR-187)', () async {
      final tree = await h.seedTree();
      final other = await h.seedTree(prefix: 'b ');
