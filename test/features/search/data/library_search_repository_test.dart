...
/// What Global Library Search finds, on a real database (BR-247, BR-250…BR-252).
void main() {
  ...
  test(
    ...
    () async {
      // BR-247 names four fields. Example, hint and pronunciation are outside the
  ...
  test(
    'a card carrying two matching tags is still one result (BR-252)',
    () async {
      final DeckEntity root = await h.root('Library');
      final DeckEntity holder = await h.child('Holder', root.id);
  ...
  test('Unicode case folds on both sides of the comparison (BR-248)', () async {
  ...
  test('an empty query opens no statement at all (BR-249)', () async {
    final DeckEntity root = await h.root('Library');
