...
/// Every state the search surface renders (wireframe M99.23 W3).
void main() {
  ...
  group('the states', () {
    testWidgets('nothing typed says what can be searched, and reads nothing', (
      ...
      expect(
        repository.calls,
        isEmpty,
        reason: 'opening the screen is not a search (BR-184)',
      );
    });
    ...
    testWidgets('decks are drawn above cards', (tester) async {
      ...
      expect(
        ...
        reason: 'BR-186 is an order on screen, not only an order in the data',
      );
    });
