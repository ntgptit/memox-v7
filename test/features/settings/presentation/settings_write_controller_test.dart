...
/// The four settings writes, as state transitions (BR-211, BR-214…BR-217).
void main() {
  ...
  group('study defaults', () {
    ...
    test('an out-of-range limit reports the typed problem and writes '
      ...
      // controller only carries which one broke (BR-211, BR-216).
      final repository = FakeAppSettingsRepository();
      final container = containerWith(repository);
    ...
    test(
      ...
      () async {
        // BR-216: the message the user sees is chosen from the failure *type*.
  ...
  group('theme and language', () {
    ...
    test('a failing theme save leaves the language controller idle', () async {
      // BR-216, S7: three groups are three transactions and three statuses. A
    ...
    test('a second submit while the first is in flight is ignored', () async {
      // BR-216's double-submit guard. `_SlowRepository` never completes its
