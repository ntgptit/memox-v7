...
/// `AppSettingsRepositoryImpl` against a real SQLite database (BR-210…BR-217).
///
...
void main() {
  ...
  /// learning state would be visible as a lost `learned_at` (BR-217).
  ...
  group('watch', () {
    ...
    test('re-emits after every write, which is how one save updates every '
      ...
      // BR-210: the surfaces subscribe, they are not told to re-read. Three
    ...
    test('an error on the stream reaches the subscriber as a Failure', () async {
      ...
      // exactly the state BR-210 calls a defect rather than a valid reading.
      final fixture = build();
      final errors = <Object>[];
  ...
  group('saveStudyDefaults', () {
    test(
      ...
      () async {
        // BR-216: one group is one transaction. Saving the study defaults must
    ...
    test('writes no deck override', () async {
      // BR-211/BR-212: the app-wide defaults and a root deck's override are two
  ...
  group('resetToDefaults', () {
    ...
    test('touches no deck override, no card and no study state', () async {
      // BR-217, and the negative half of it is the point: this app has a second
  ...
  group('the single row', () {
    test('a second row cannot be inserted', () async {
      ...
      // "whichever row was read first" (BR-210).
      final fixture = build();
