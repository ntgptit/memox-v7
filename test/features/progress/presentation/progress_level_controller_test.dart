...
void main() {
  ...
  group('the read it opens', () {
    test('passes the caller-owned clock and offset down', () {
      ...
      expect(repository.reads.single.now, start);
      expect(repository.reads.single.utcOffset, progressTestOffset);
      expect(repository.reads.single.deckId, isNull);
    });
    ...
    test('the family keys the read by deck, so one level does not disturb '
      ...
      expect(repository.reads.map((read) => read.deckId).toList(), <String?>[
        'deck-1',
        null,
      ]);
    });
  ...
  group('the day-boundary wake-up (BR-184)', () {
    testWidgets('re-measures at the next local midnight and not before', (
      ...
      expect(repository.readCount, greaterThan(readsBefore));
    ...
    testWidgets('a boundary already behind us refreshes once, not in a loop', (
      ...
      final afterFirstChase = repository.readCount;
      await tester.pump(const Duration(seconds: 1));
      await tester.pump(const Duration(seconds: 1));

      expect(repository.readCount, afterFirstChase);
    });
