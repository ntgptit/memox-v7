    ...
    useCases: <WidgetbookUseCase>[
      WidgetbookUseCase(
        ...
        builder: (BuildContext context) {
          ...
          return CatalogCenterPage(
            child: MxIconButton(
              // The flag is the tone axis's first caller, so the catalog shows
              // the glyph pair it changes with rather than a neutral icon: a
              // toned button that keeps the same shape would suggest colour is
              // the whole signal, which is exactly what it must not be.
