/* FlashcardList · state: cardActions — card ⋮ / long-press → open detail (read-only) · edit · flag · move · Trash (A8; opening never edits by default, BR-246). ADDED in v3. */
(function () {
const R = (window.MemoXStates = window.MemoXStates || {});
const D = (R.FlashcardList = R.FlashcardList || {});
D.cardActions = function (ctx) {
  const { Ic, Scrim, BottomSheet, CardList } = ctx;
  const rows = [
    { ic: 'eye', label: 'Open card', sub: 'Content, schedule and history' },
    { ic: 'pencil', label: 'Edit' },
    { ic: 'flag', label: 'Flag' },
    ...
    <>
      ...
      <BottomSheet scrim={false} maxHeight="none">
        <div style={{ padding: '4px 16px 8px' }}>
          <div style={{ fontSize: 16, fontWeight: 700, letterSpacing: '-0.2px' }}>먹다</div>
          <div style={{ fontSize: 12, color: 'var(--memox-on-surface-variant)', marginTop: 4 }}>ăn · Beginning · due today</div>
        </div>
