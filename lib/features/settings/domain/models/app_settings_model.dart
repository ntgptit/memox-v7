...
/// signature that the bounds have been applied (BR-183). A value read back out
...
///
/// **`NewCardOrder` is reused, not redeclared** (BR-183). A second enum with
...
abstract class AppSettingsModel with _$AppSettingsModel {
  const factory AppSettingsModel({
    ...
    /// The theme the user picked, not the brightness it resolves to (BR-186).
    required AppThemeMode themeMode,

    /// The language the user picked, not the locale it resolves to (BR-187).
    required AppLanguage language,
  }) = _AppSettingsModel;

  /// The state a fresh install and a completed reset both land in (BR-189).
  ///
  ...
  /// retyped here: BR-183 requires exactly one copy of the number, and this is
