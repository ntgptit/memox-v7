...
/// Everything Trash does to a batch after it exists (UC-21).
///
...
abstract interface class TrashRepository {
  /// Every batch, newest first (UC-21 step 3).
  ///
  ...
  /// Where [batchId] may be restored to, live (BR-261).
  ///
  ...
  /// Restores [batchId] into [target], atomically — BR-261, BR-262.
  ///
  /// Revives exactly the rows carrying this batch — a descendant tombstoned by
  /// an older batch stays where it is (BR-258) — re-parents the item root,
  ...
  /// Puts [batchId] back exactly where it was, without asking (BR-263).
  ///
  ...
  /// Purges everything past retention, idempotently (BR-264).
  ///
