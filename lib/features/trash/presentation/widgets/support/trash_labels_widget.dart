...
/// BR-187 says restore reuses the move *rule set*, so a restore refused by
...
extension TrashLabels on BuildContext {
  /// Copy for a refusal only Trash can produce (UC-12 E1…E4).
  ...
  /// Why a restore target was refused (UC-09 step 2, reused by BR-187).
  String trashMoveRejection(DeckMoveRejection rejection) => switch (rejection) {
    ...
    // (BR-187) — but present because every value must have text.
  ...
  /// The original path, as a single line of context (BR-193, wireframe T5).
  ///
  ...
  /// What a deck batch would bring back (BR-188).
