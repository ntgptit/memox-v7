...
/// (BR-261). What is not shared is the *loading*: a move reads a live source
...
mixin _TrashRestoreOperations implements TrashRepository {
  ...
  /// One reason for every way Undo can be refused (BR-263).
  ///
  ...
  Future<void> _restoreCard(
    ...
    // The target gains its first card, in the same step (BR-62, BR-261).
  ...
  Future<void> _restoreDeck(
    ...
    // **Tombstones inside the subtree move too** (BR-262). A branch deleted
  ...
  /// A root deck goes back to being a root — nothing to re-parent (BR-261).
