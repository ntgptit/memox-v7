...
/// Deck or Card repository's own delete transaction (BR-182), and a nested
...
final class ContentTrashRepositoryImpl implements ContentTrashRepository {
  ...
  Future<String> markDeckDeleted(String deckId) => _guard(() async {
    ...
    // keeps the batch it went with and is not swept into this one (BR-184).
    ...
    // BR-185, in the same transaction and before the rows go dark. Both id sets
  ...
  Future<List<String>> markCardsDeleted(List<String> cardIds) =>
      _guard(() async {
        ...
        // **One batch per card, not one per action** (BR-182). The item root is
        ...
        // partial restore that BR-188 does not have.
