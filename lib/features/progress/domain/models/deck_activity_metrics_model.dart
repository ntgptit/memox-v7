...
/// What one deck's subtree amounts to inside one window (BR-183…BR-186).
///
/// **Four figures, and only four.** Accuracy, score, streak and due forecast are
/// deliberately out of v1 (BR-182): each needs its own definition argument, and
...
/// work itself. The pair is exhaustive and exclusive by construction (BR-186),
...
abstract class DeckActivityMetrics with _$DeckActivityMetrics {
  const factory DeckActivityMetrics({
    /// Distinct cards answered at least once in the window (BR-183).
    required int activeCardCount,

    /// Distinct local days on which at least one card was answered (BR-183).
    ///
    ...
    /// Card-days whose day contained at least one `learning` turn (BR-186).
  ...
  /// (BR-186) — a card-day is one or the other, never both and never neither.
