...
class _StudyEntryScreenState extends ConsumerState<StudyEntryScreen> {
  ...
  /// algorithm and the mode make that a real question (BR-182).
  ///
  ...
  Future<void> _startReview(
    ...
    return showModalBottomSheet<void>(
      context: context,
      // The choice is locked for the session (BR-186), so it is confirmed rather
  ...
  ///
  /// **The re-read is not ceremony, and it checks two things because UC-12 has
  ...
  /// this is the user-facing one, and it is what UC-12 promises.
  ///
  ...
  Future<void> _open(
    ...
    /// The recall direction chosen for this session (BR-182). Null for every
    /// path the rule does not cover, and for a resume (BR-186).
