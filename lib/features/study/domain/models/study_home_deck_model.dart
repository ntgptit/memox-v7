...
/// One root deck on the Study tab, with the workload it is ranked by (BR-183).
///
...
///
/// **Three counts, not one total** (BR-182, BR-162). They cost differently — a
...
abstract class StudyHomeDeckModel with _$StudyHomeDeckModel {
  const factory StudyHomeDeckModel({
    ...
    /// nothing here" — two states with different next steps (BR-184).
  ...
  ///
  /// **Cards, not workload** (BR-183). A deck with nothing due is still a deck
...
/// The order the Study tab lists decks in (BR-183).
///
