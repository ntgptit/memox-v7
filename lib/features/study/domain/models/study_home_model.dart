...
/// Everything the Study tab shows, from one read (AD-13, BR-200).
///
...
/// no session, no scheduler lock, no materialised queue (BR-101, BR-200).
@freezed
abstract class StudyHomeModel with _$StudyHomeModel {
  const factory StudyHomeModel({
    ...
    /// Every root deck of the library, already in the order BR-201 defines.
    ///
    ...
    /// empty", and those two states have different next steps (BR-202).
  ...
  /// way forward is ready-made content (BR-202, UC-01).
  ...
  /// is the Library rather than the starter catalog (BR-202).
  bool get hasNoCards =>
      decks.isNotEmpty && decks.every((deck) => deck.totalCardCount == 0);

  /// The decks the list actually offers, in order (BR-201).
  ///
