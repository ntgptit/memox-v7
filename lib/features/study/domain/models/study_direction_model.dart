...
/// The way **one turn** actually asks (BR-183).
///
...
enum StudyRecallDirection {
  ...
  /// the only thing the app could measure before BR-182.
  ...
  /// The face the learner is asked from (BR-183).
  ///
  /// [unknown] falls back to the front, which is what every build before BR-182
...
/// What the learner chose before the session started (BR-182).
///
...
/// silence. Korean→Meaning is what every review did before BR-182, so taking the
...
/// The one place the feature's reachability is decided (BR-182).
///
...
/// One direction per card of a [StudySessionDirection.mixed] queue (BR-184).
///
