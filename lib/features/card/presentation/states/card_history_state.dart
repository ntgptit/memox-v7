...
/// What the history timeline is doing right now (UC-12 UI states).
///
...
enum CardHistoryStatus {
  ...
  /// tail offers `Try again` (UC-12 E4).
  pageError,
}
...
/// last row read (BR-184), so the only place that can hold it correctly is the
...
class CardHistoryState {
  ...
  /// Newest first, accumulated across pages (BR-184).
  ...
  /// (BR-187). Not an error, and deliberately not the same thing as
