...
/// The paginated review history of one card (UC-19 steps 4–6, BR-241).
///
...
/// duplicate BR-241 forbids. So the rows live in state and each request appends
/// to them.
///
/// **No two requests are ever in flight at once, and that is what makes the
/// appending safe** (UC-19 E5). [loadMore] writes the in-flight status into
...
class CardHistory extends _$CardHistory {
  ...
  /// Fetches the page after whatever is on screen (UC-19 steps 4–6, E4).
  ///
  ...
  /// disjoint (BR-241); this is the cheap second line that keeps a bug in the
