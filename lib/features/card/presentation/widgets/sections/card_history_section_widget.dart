...
/// The review timeline (BR-184…BR-187, M4.14 W2 band 3).
///
/// **Grouped by the generation stored on each row, with a text heading**
/// (BR-186, M4.14 V6). A Reset keeps the history but cuts its meaning in two: a
...
/// A card with no reviews yet — valid, and never an error (BR-187).
...
/// The heading above one generation's events (BR-186).
  ...
  Widget build(BuildContext context) {
    ...
    if (state.isComplete) {
      return Padding(
        ...
        // total", which BR-186 forbids — and it was already wrong: `Reviews` in
