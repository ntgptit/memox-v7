...
/// Drift-backed [TagCatalogRepository] (UC-18).
///
...
/// to a `cards` row, and that is BR-236 made structural rather than promised —
...
final class TagCatalogRepositoryImpl implements TagCatalogRepository {
  ...
  Future<TagRenameOutcome> renameTag({
    ...
    () => _dao.runInTransaction(() async {
      ...
      // the same identity, so it is a re-spelling and not a merge (BR-233).
  ...
  Future<void> deleteTag(String tagId) => _guard(
    () => _dao.runInTransaction(() async {
      ...
      // by relying on the cascade** (BR-235). Cards are not touched at all:
  ...
  /// BR-234's merge, inside a transaction the caller already opened.
  ///
