...
/// management surface never touches (`study_answers`) and is, by BR-239,
...
abstract interface class CardDetailRepository {
  ...
  /// the stream (BR-245). The same reason the editor already uses, not a second
  /// one: it is the same event.
  Stream<CardDetailModel> watchCardDetail(String cardId);

  /// One page of the card's history, newest first (BR-241).
  ///
