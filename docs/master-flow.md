...
| **Scope** | Đồ thị chuyển tiếp giữa UC-01…UC-12, tách theo đối tượng nghiệp vụ. Ngoài phạm vi: nội dung của từng UC, mọi luật nghiệp vụ, và mọi chi tiết màn hình |
...
| **Updated by task** | M99.23 — nhánh `J`: chạm một hàng card mở chi tiết chỉ đọc (UC-12), Edit thành action tường minh; bảng §6 bổ sung UC-10…UC-12 |
...
flowchart TD
    ...
    C -->|"Chạm một hàng"| J["Chi tiết card, chỉ đọc · UC-12"]
    J --> J1["Lịch sử học phân trang keyset, nhóm theo generation · UC-12, BR-184, BR-186"]
...
M99.23, chạm một hàng ở chế độ thường mở **chi tiết chỉ đọc**, không mở editor
nữa; đường tới `H` đi qua một action `Edit` tường minh. Trong chế độ chọn nhiều
thì chạm vẫn chỉ là chọn/bỏ chọn và **không** có đường nào tới `J` (BR-189).
...
| UC-12 | card | M99.23 | Đủ — chi tiết chỉ đọc cộng lịch sử phân trang keyset |
