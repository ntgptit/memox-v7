...
| `study_session_controller.dart` vượt trần 400 dòng của guard | M5.24 | 423/400. Warning cũng làm đỏ gate. Class giữ toàn bộ command của phiên học, và recovery cho leave-thất-bại vừa thêm ~30 dòng | **Không tách được — đã kiểm chứng chứ không suy đoán.** Dart không có partial class; base class Riverpod sinh ra là private nên không `mixin ... on` được; và **extension trong một `part` cũng không dùng được `state`** — analyzer báo `invalid_use_of_protected_member` + `invalid_use_of_visible_for_testing_member` (đã thử, đã revert). Đã vắt hết phần vắt được qua bốn PR: `_submit` chung, `_writeFailed` chung, `GetNextStageTurnUseCase`, `OpenStudySessionUseCase`, `StudyOperations`, summary thành query provider, nén prose nhiều vòng. Lối ra còn lại là **tách một command cluster thành controller thứ hai chia sẻ state** — một quyết định thiết kế có rủi ro race riêng, nên phải là task độc lập chứ không phải phần đuôi của một task recovery |
