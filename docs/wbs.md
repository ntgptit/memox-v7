...
| `study_session_controller.dart` vượt trần 400 dòng của guard | M5.23 | 408/400, và **warning cũng làm đỏ gate** — `dod_check.sh` coi guard là fail. Class giữ toàn bộ command của phiên học: start · submitAnswer · advance · browseStep · pause · leave, cộng summary và failure policy | **Không tách được bằng cách dời file.** Dart không có partial class, và `_$StudySessionController` do Riverpod sinh ra là private nên không `mixin ... on` nó được. Đã vắt hết phần vắt được: một `_submit` chung, một `_writeFailed` chung, bỏ nhánh `browse` khỏi `answer()`, dời `StudyBrowseStep` sang state, dời `_nextTurn` thành `GetNextStageTurnUseCase`, nén prose hai vòng. Bước tiếp theo là **tách summary thành query controller riêng** (`_loadSummary` + `StudySessionState.summary`) — đó là một query, và CQS test đã có hạng mục cho nó. Việc đó đổi state shape nên phải là task riêng, không phải phần đuôi của một task lifecycle |
