...
- **Status:** done — **superseded by M4.10d** trong cùng PR, trước khi merge:
...
### M4.10d · Widgetbook catalog — thay màn showcase in-app

- **Status:** done
- **Goal:** Cùng nội dung M4.10c nhưng ở dạng chuẩn công nghiệp: mỗi component
...
- **Scope:** package `widgetbook/` mới (app riêng, web); 3 trang token +
  ...
  **Compact 320×568** (M4.8b) không preset nào có sẵn; gỡ toàn bộ M4.10c khỏi
...
- **Demo overlay vẫn render inline** (không `showDialog`/`showModalBottomSheet`)
  ...
  ListTile/Material đã ghi ở M4.10c.
