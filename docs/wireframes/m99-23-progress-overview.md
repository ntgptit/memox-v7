...
| **Updated by task** | M99.23 (stage 1 của batch tích hợp #301–#310 — recursive UI/UX review vòng 1 và 2) · M99.24 (thu phạm vi về phần đầu sau khi `/progress` hợp nhất) · M100.89 (đo lại X7 sau khi bỏ Inter) |
...
| X7 | Cột bar tụt dưới sàn tự khai `content / 4` khi một ngày vượt ~1000 card | Đo lại ở `320 @ 2.0 · vi` sau M100.89 (Plus Jakarta Sans, gutter 16 ở 320): busiest 143 → **78.7dp**, 999 → **73.7dp**, 1234 → **61.9dp**, 2000 → **45.5dp**, 9999 → **56.9dp**, 12345 → **44.7dp**, so với sàn **64.0dp** (bản Inter với gutter 12: 1234 → 63.8dp, 12345 → 46.9dp, sàn 66.0dp). Không overflow, không exception — `IntrinsicColumnWidth` của cột giá trị chỉ lặng lẽ ăn dần cột flex | v1 **không** bảo đảm sàn cho ngày > 999 card. Sửa thật là một quyết định copy/thiết kế — giới hạn bề rộng cột giá trị, hoặc rút gọn số thành `1.2k` — nên nó không thuộc phần tích hợp này; owner ở `docs/wbs.md` M99.23 deferred debt 5. Ghim ở `progress_screen_geometry_extremes_test.dart` — không phải ghim cái sàn, mà ghim **con số đã chấp nhận** ở 1234 card, để ngày sàn được sửa thật thì test đỏ có chủ đích |
