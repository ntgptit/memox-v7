# Wireframe M99.23 — Trash và restore (list → target → confirm)

| | |
|---|---|
| **Status** | active |
| **Purpose** | Chốt cấu trúc UI của Trash để xây mà không phải đoán layout, copy, geometry hay state nào |
| **Scope** | Entry point, danh sách Trash, chế độ chọn, picker target, hộp thoại purge, snackbar Undo, hợp đồng geometry và responsive/a11y. Ngoài phạm vi: luật nghiệp vụ (BR-182…BR-193), luồng (UC-12), mô hình tombstone (AD-21) |
...
| **Depends on** | `../use-cases.md` (UC-12), `../business-rules.md` (BR-182…BR-193), `../architecture.md` (AD-21), `m4-11-card-management.md` |
...
| T5 | Đường dẫn gốc là **dòng phụ, chữ nhỏ, không có affordance**; không phải nút, không phải link | BR-193 — nó là ngữ cảnh, không phải nơi item sẽ về. Một đường dẫn trông bấm được nói ngược lại BR-187 trước khi người dùng kịp đọc dialog | 2026-08-13 |
...
| T7 | Chọn nhiều **khoá theo loại của item đầu tiên**: chọn một card thì mọi hàng deck mất khả năng chọn và ngược lại; thanh chọn nói rõ lý do | BR-192. Hiện checkbox rồi từ chối lúc submit là dạy người dùng một quy tắc bằng một lỗi | 2026-08-13 |
| T8 | Picker target là **bottom sheet**, dựng từ đúng eligibility của move; **không** có hàng bị vô hiệu hoá | BR-187, E1 của M4.13 cùng lập luận. Một hàng xám vẫn là một hàng phải đọc, và nó không nói được vì sao nó xám | 2026-08-13 |
| T9 | Picker **preselect** một target hợp lệ khi vị trí cũ vẫn hợp lệ, nhưng primary vẫn phải bấm | BR-188 — preselect tiết kiệm một lần chạm mà không biến im lặng thành đồng ý | 2026-08-13 |
| T10 | Chỉ `Delete permanently` mang vai trò màu destructive; `Restore` và mọi thứ khác dùng vai trò bình thường | BR-192. Nếu soft-delete cũng đỏ thì màu đỏ không còn nghĩa gì ở đúng chỗ nó phải có nghĩa | 2026-08-13 |
| T11 | Hộp thoại purge nêu **đúng số item**, nói lịch sử học không khôi phục được, và đặt focus mặc định ở `Cancel` | BR-192. Focus mặc định ở hành động phá huỷ biến phím Enter thành một cách mất dữ liệu | 2026-08-13 |
| T12 | Batch hết hạn biến mất **tại chỗ** khi auto-purge chạy; danh sách không cuộn lại đầu và không hiện spinner | UC-12 A4. Danh sách là một `watch()` stream, nên thứ đúng để xảy ra là một hàng biến mất, không phải một lần tải lại | 2026-08-13 |
| T13 | Undo là **snackbar tại màn vừa xoá**, không phải một banner trong Trash | Undo chỉ có nghĩa khi người dùng còn ngữ cảnh của thao tác (BR-189). Trong Trash thì hành động đúng đã là `Restore` | 2026-08-13 |
