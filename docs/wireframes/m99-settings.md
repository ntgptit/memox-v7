# Wireframe M99.23 — Settings v1 (study defaults · appearance · language)

| | |
|---|---|
| **Status** | active |
| **Purpose** | Chốt cấu trúc UI của màn Settings toàn app để M99.23 xây mà không phải đoán layout, copy, geometry hay state nào |
| **Scope** | Màn `/settings`: anatomy, ba nhóm tuỳ chọn, hành động reset, mọi trạng thái, hợp đồng geometry, responsive/a11y. Ngoài phạm vi: luật nghiệp vụ (BR-182…BR-189), luồng (UC-12), branch contract (AD-19), và surface tuỳ chọn của deck — nó thuộc `StudyOptionsScreen` và chỉ được nhắc ở S6 |
...
| **Depends on** | `../use-cases.md` (UC-12), `../business-rules.md` (BR-182…BR-189), `../architecture.md` (AD-15, AD-19), `m4-13-card-export.md` |
| **Updated by task** | M99.23 (phase 8 — recursive UI/UX review) |
...
| S2 | **Theme và ngôn ngữ ghi ngay khi chạm; study defaults có nút `Save`** | Không phải sự thiếu nhất quán mà là hai loại input khác nhau. Theme và ngôn ngữ là lựa chọn rời rạc và **tự xác nhận** — người dùng thấy kết quả ngay trên chính màn hình đó, nên một nút Save chỉ thêm một bước giữa hành động và bằng chứng. Trần thẻ là văn bản tự do: nó có trạng thái nửa chừng (`2` trên đường gõ `20`) và có thể sai, nên nó cần một mốc submit rõ ràng (BR-188) | 2026-08-13 |
| S3 | `New card order` đi **cùng nhóm** với trần thẻ và **cùng nút Save**, dù nó là lựa chọn rời rạc | Hai giá trị này là một cặp trong đầu người dùng ("một phiên trông như thế nào") và là một cặp trong dữ liệu (BR-183). Tách nút Save cho một nửa nghĩa là chạm order rồi rời màn hình sẽ lưu một nửa những gì họ vừa đổi — đúng loại "lưu một phần" mà BR-188 cấm | 2026-08-13 |
| S4 | Dòng BR-185 nằm **trong** nhóm Study defaults, ngay trên nút `Save`, không phải một banner đầu màn | Nó chỉ đúng cho nhóm đó. Đặt ở đầu màn thì nó cũng nói về theme và ngôn ngữ, vốn áp dụng tức thì — một câu vừa đúng vừa sai tuỳ người đọc đang nhìn nhóm nào | 2026-08-13 |
| S5 | `Reset to defaults` là **hành động chữ**, không phải card đỏ, và có bước xác nhận nói rõ nó **không** đụng tiến độ học | Đúng idiom `_DestructiveRow` mà `test/design_preview/settings_preview_test.dart` đã chốt: `danger` là màu **chữ**, không phải nền. Một khối đỏ ở cuối mọi màn Settings kéo mắt về phía dưới và đọc như một lỗi app đang báo. Câu xác nhận phải nói phạm vi vì hai hành động khác nhau ở app này có cùng chữ "reset" (BR-42 vs BR-189) | 2026-08-13 |
| S6 | `Use app defaults` của root deck **không** xuất hiện trên màn này | Nó tác động lên **một** deck; đặt nó ở màn toàn app thì phải kèm một deck picker, tức là một màn hình thứ hai trong màn hình này. Nó sống ở `StudyOptionsScreen`, nơi deck đang mở là ngữ cảnh sẵn có (BR-184) | 2026-08-13 |
| S7 | Trạng thái `saving` khoá **control của nhóm đang ghi**, không khoá cả màn | Ba nhóm là ba transaction rời nhau (BR-188). Một overlay toàn màn biến ba lần ghi độc lập thành một hàng đợi mà người dùng phải nhìn, và với một lần ghi SQLite cục bộ thì nó nhấp nháy chứ không thông báo | 2026-08-13 |
...
| S9a | **Thứ tự thẻ mới trên màn này khác `StudyOptionsSectionWidget`, và đó là một lệch có chủ đích.** Màn tuỳ chọn của deck vẫn dùng pill | Bản đầu của S3/W5 yêu cầu hai màn "không được nhìn khác nhau", và lập luận đó đúng cho *nhịp và nhãn* nhưng sai khi nó kéo theo cả một control vi phạm W6. Sửa đúng chỗ là ở `MxPillButton`/`app_chip_theme.dart` — và ở kit web phản chiếu nó — chứ không phải nhét một câu trả lời riêng vào một feature; nên gap đó được ghi lại chứ không sửa lén ở đây. Xem ghi chú M99.23 trong `docs/wbs.md` | 2026-08-14 |
...
│  │ ( ) Shuffled                  │  │
│  │                               │  │
│  │ Applies to sessions you start │  │  ← BR-185, bodySmall
│  │ after saving.                 │  │
│  │ [        Save              ]  │  │  ← MxActionButton
│  └───────────────────────────────┘  │
│                                     │
...
| Ghi chú BR-185 | `Applies to sessions you start after saving. A session already running keeps its own limit.` | `Áp dụng cho phiên bạn bắt đầu sau khi lưu. Phiên đang chạy giữ nguyên trần của nó.` |
...
| # | Trạng thái | Màn hình |
|---|---|---|
| 1 | loading | `MxLoadingState` toàn thân. Không nhóm nào vẽ trước — một control hiện `System` trong lúc chưa đọc xong là một giá trị bịa (BR-182) |
...
| 5 | validation error | Chỉ nhóm Study defaults có được. Message dưới trường trần thẻ, draft giữ nguyên, không ghi gì (BR-183, BR-188) |
| 6 | persistence error | Dải lỗi trong nhóm hỏng + `Try again`. Các control còn lại hiển thị **giá trị đã persisted**, không phải giá trị vừa chạm hụt (BR-188) |
...
gì. Xác nhận là một submit chịu đúng BR-188: khoá double submit, và lỗi hiện
...
thứ hai do màn hình tự gọi (BR-182).
...
- Đổi theme hoặc ngôn ngữ MUST áp ngay, MUST NOT nháy sang một theme thứ ba
  giữa chừng, và MUST NOT đưa người dùng về đầu màn hay về tab khác (BR-186,
  BR-187).
...
- Hành động reset MUST NOT dùng chữ khiến nó bị nhầm với Reset learning progress
  (BR-189), và phụ đề của nó là câu nói rõ điều đó — nên phụ đề MUST hiện ở
