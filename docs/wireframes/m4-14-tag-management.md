...
|---|---|
| **Status** | active |
| **Purpose** | Chốt cấu trúc UI của tag catalog và overlay lọc theo tag để M99.23 xây mà không phải đoán layout, copy, geometry hay state nào |
| **Scope** | Tag catalog screen, overlay rename (có mặt gộp), dialog xoá, overlay lọc nhiều tag, và mặt "card list đang lọc theo tag". Ngoài phạm vi: luật nghiệp vụ (BR-182…BR-190), luồng (UC-12), card list nói chung (`m4-11-card-management.md`) |
...
| **Depends on** | `../use-cases.md` (UC-12), `../business-rules.md` (BR-182…BR-190), `m4-11-card-management.md` |
| **Updated by task** | M99.23 (bổ sung T3a, W1, W7 và G10 sau hai vòng review) |
...
| T2 | Hai entry point, cả hai **nhìn thấy được**: icon `Tags` trên app bar của Library, và mục `Manage tags` trong overflow menu của card list | BR-182 nói catalog là library-level nên lối vào chính thuộc Library. Card list vẫn cần lối vào vì đó là nơi người dùng nhận ra một tag viết sai. Long-press **không** phải lối vào — nó không có affordance nào | 2026-08-13 |
...
| T5 | Overlay lọc là **bottom sheet có draft**: chọn nhiều, `Clear`, `Apply` | Áp ngay mỗi lần chạm khiến danh sách phía sau nhảy N lần cho một ý định, và mỗi lần nhảy là một lần reset cửa sổ (BR-184). Draft cũng là chỗ duy nhất `Clear` có nghĩa rõ ràng | 2026-08-13 |
...
| T7 | Gộp **không** có hành động riêng; nó là hệ quả của rename và được **tiết lộ trước khi xác nhận** | BR-186. Người dùng gõ `Noun` lên `nouns` là đang nói hai cái này là một. Nhưng gộp là mất một tag, nên nó phải được nói ra trước, không phải phát hiện sau | 2026-08-13 |
| T8 | Copy xoá nói **gỡ tag khỏi N thẻ**, không bao giờ nói mất thẻ; nút phá huỷ là `Delete tag` chứ không phải `Delete` | BR-187. `Delete` trần trong một app có thẻ đọc như "xoá thẻ". Một danh từ trong nhãn nút là chi phí rẻ nhất để không ai mất dữ liệu vì một câu mơ hồ | 2026-08-13 |
...
| T10 | Tag có 0 thẻ **vẫn hiện**, với số đếm `0` | BR-182. Nó là thứ người dùng vào đây để dọn; ẩn nó đi là ẩn đúng hàng mà catalog tồn tại để xử lý | 2026-08-13 |
...
- **Library app bar:** icon `sell_outlined`, `semanticLabel` = `Manage tags`
  ...
  rỗng là một câu trả lời, không phải một lỗi (T2, BR-182).
...
3. **Dải tiết lộ gộp** — chỉ hiện khi tên đã gõ fold trùng một tag **khác**
   (T7, BR-186). Kiểu info, icon `merge`, copy nêu **tên tag đích** và nói rõ
...
5. Lỗi validation (BR-93) gắn **dưới ô nhập**, không phải một dải riêng: nó nói
   về chữ vừa gõ.
6. Lỗi thao tác (E3, E4 của UC-12) hiện thành **dải lỗi trong sheet**, giữ
...
- **Nút phá huỷ** `Delete tag`; `Cancel` là mặc định (T8, BR-187).
...
5. Đóng overlay mà không `Apply` bỏ draft, giữ nguyên tập đang áp (UC-12 A5).
...
- Không có kết quả: dùng mặt `no match` đã có của card list (M4.11), vì lý do
  ...
  **Khi thứ làm rỗng danh sách là vị từ tag**, mặt đó mang thêm một action
  `Clear` bỏ toàn bộ tag đang chọn (UC-12 A7). Thêm một action vào mặt đang có
  ...
  vào mặt người dùng.
- Đổi tập tag reset cửa sổ và xoá selection (BR-184); không có hiệu ứng nào
  khác trên màn.
