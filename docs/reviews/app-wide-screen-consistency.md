...
| ID | Kết luận cuối | Vì sao |
|---|---|---|
| `SC-C3-01` | **FIXED** | Reproduces, with a line offset. lib/features/deck/presentation/screens/starter_library_screen.dart:46-50 is `MxErrorState(title: context.l10n.starterLibraryTitle, message: context.l10n.starterLibraryInstallFailed, onRetry: () => r… |
| `SC-C3-02` | **FIXED** | All four call sites reproduce on one screen: starter_library_screen.dart:36 (shell title), :44 (loadingLabel), :47 (error title), :68 (empty title) are all `context.l10n.starterLibraryTitle`. app_en.arb:344 = "Starter library". Th… |
| `SC-C3-03` | **REVISED_AND_FIXED** | The defect reproduces. card_create_form_widget.dart:106-112 renders `Text(context.l10n.cardEditorSaveFailed, style: context.texts.bodyMedium!.inked(context, AppInk.error))` bare, while the identical string in edit mode at card_edi… **Target đã đổi:** Wrap the existing `Text` at card_create_form_widget.dart:108-111 in `Semantics(liveRegion: true)`, mirroring card_editor_form_widget.dart:117-123 exactly — same widget, same `bodyMedium`, same `AppInk |
| `SC-C3-04` | **REVISED_AND_FIXED** | Measurements all reproduce. deck_list_screen.dart:78-79 `loadingFrame: (loading) => MxContentShell(title: _titleBeforeData(context), body: loading)` — no actions, no titleSubline, no FAB; deck_level_error_widget.dart:45-61 is the … **Target đã đổi:** Split by level. (a) Both frames, both levels: pass the search action, which depends on nothing — `MxIconButton(icon: Icons.search, semanticLabel/tooltip: l10n.librarySearchOpenLabel, onPressed: () =>  |
| `SC-C3-05` | **REVISED_AND_FIXED** | The duplication is real: deck_list_screen.dart:310-327 gives the root empty state `actionLabel: deckStarterLibraryAction` ("Starter library", app_en.arb:340) as primary and `secondaryActionLabel: deckCreateRootAction` ("New deck",… **Target đã đổi:** Take option (b), the card list's pairing: at deck_list_screen.dart:310-327 make the root empty state's primary `actionLabel: context.l10n.deckCreateRootAction` / `onAction: () => showCreateRootDeckFor |
| `SC-C3-06` | **FIXED** | Reproduces exactly. deck_list_screen.dart:234-237 states the policy ("a filter and a sort over nothing are two controls that visibly do nothing, which is exactly the dead control this design refused to copy") and returns `_emptyLe… |
| `SC-C3-07` | **FIXED** | Reproduces with a small line offset. study_options_screen.dart:46 puts `context.l10n.studyOptionsTitle` in the shell's app bar and :56-59 is `MxErrorState(title: context.l10n.studyOptionsTitle, message: context.l10n.studyOptionsNe… |
| `SC-C3-08` | **FIXED** | Both halves reproduce, at the cited lines. study_options_section_widget.dart:117-121 is `MxActionButton(label: l10n.studyOptionsSave, isLoading: widget.isSubmitting, onPressed: () => widget.onSave(_cardLimit.text, _order))` — a no… |
| `SC-C3-09` | **REVISED_AND_FIXED** | The defect is real and the wireframe is on the finding's side. trash_screen.dart:77-82 decides the subheader from `selection.isActive` alone, so the chips render over `_TrashBody`'s `MxEmptyState` at :157-169 (`trashEmptyTitle` = … **Target đã đổi:** Hide the band only once the read has answered and answered empty, not while it is in flight: `subheader: (selection.isActive \|\| (batches.hasValue && batches.value!.isEmpty)) ? null : TrashFilterBarWid |
| `SC-C3-10` | **REVISED_AND_FIXED** | The facts hold. trash_screen.dart:77-82 unmounts TrashFilterBarWidget the moment `selection.isActive`, with no comment justifying it, while `_TrashBody` keeps applying `filter.admits(batch.itemType)` at :152-155 and the Select act… **Target đã đổi:** Keep the band mounted through selection but inert: `subheader: (batches.value?.isEmpty ?? false) ? null : TrashFilterBarWidget(filter: filter, onChanged: selection.isActive ? null : (value) => _setFil |
| `SC-C3-11` | **FIXED** | Reproduces at the cited lines. tag_catalog_screen.dart:71-76 is `MxErrorState(title: unexpectedErrorTitle, message: tagCatalogError, retryLabel: retryAction, onRetry: () => ref.invalidate(tagCatalogProvider))` with no `isRetrying`… |
| `SC-C3-12` | **REVISED_AND_FIXED** | The copy half reproduces exactly: study_session_screen.dart:270-275 returns `MxErrorState(title: context.l10n.appTitle, message: context.l10n.studyNothingDueMessage)` — app_en.arb:3 `appTitle` = "MemoX", app_en.arb:1754 = "Nothing… **Target đã đổi:** Keep the copy fix — new `studySessionErrorTitle` / `studySessionErrorMessage` in both ARBs, and `studyNothingDueMessage` stays with the empty state it belongs to. Change the action to the way out this |
| `SC-C3-13` | **REVISED_AND_FIXED** | The defect reproduces, at :277, :324 and :330 (the finding cites :277/:313/:319 — line drift only). All three are `MxLoadingState(semanticsLabel: context.l10n.appTitle)` and app_en.arb:3 is "MemoX". A repo-wide grep of `loadingLab… **Target đã đổi:** Two keys, not one. `studySessionLoadingLabel` ("Opening study session…") at :277 (`isOpening`) and :330 (session/turn not yet resolved); a separate `studySummaryLoadingLabel` ("Loading your session su |
| `SC-C3-14` | **FIXED** | Reproduces at :131-135 (finding cites :130-134; one-line drift). `loadingLabel: context.l10n.appTitle`, then `MxErrorState(title: context.l10n.appTitle, message: context.l10n.studyNothingDueMessage)` with no retry and no live regi… |
| `SC-C3-15` | **REVISED_AND_FIXED** | The state reproduces. study_entry_section_widget.dart:58-75: both branches fall through to `Text(l10n.studyNothingNewMessage)` (app_en.arb:1758 "Every card here has been learned.") and `Text(l10n.studyNothingDueMessage)` (app_en.a… **Target đã đổi:** At `newCount == 0 && dueCount == 0`, render `MxEmptyState(title: <new studyEntryAllCaughtUpTitle>, message: context.l10n.studyNothingDueMessage)` with **no** action and no counts row, following trash_ |
| `SC-C3-16` | **FIXED** | Reproduces. card_detail_screen.dart:161-185 `_failureFace` returns a bare `MxEmptyState` (:165-176, `cardDetailNotFoundTitle`) or a bare `MxErrorState` (:179-184, `unexpectedErrorTitle`), and it is called at :149 inside `error:` w… |
| `SC-C3-17` | **FIXED** | Reproduces. card_detail_screen.dart:179-184 passes `retryLabel` + `onRetry: () => _retryDetail(ref, cardId)` and no `isRetrying`; `_retryDetail` is `ref.invalidate(cardDetailProvider(cardId))` at :30-31, and the provider is a stre… |
| `SC-C3-18` | **REVISED_AND_FIXED** | The mechanism reproduces exactly. progress_deck_screen.dart:299-301 `_emptyLevelWithHeader` returns `ProgressHeaderedBody`, which at progress_level_header_widget.dart:68-76 is `SingleChildScrollView(child: Column(crossAxisAlignmen… **Target đã đổi:** Keep the structural change — it is correct and it is what the loaded branch and the deck list already do — but sequence and test it honestly. Make `ProgressHeaderedBody` a `CustomScrollView(slivers: [ |
| `SC-C3-19` | **REVISED_AND_FIXED** | All five sites reproduce (line drift on two): deck_form_widget.dart:138-144 (gap `md`), move_deck_sheet_widget.dart:77-83 (gap `sm`), deck_scheduler_change_widget.dart:193-199 (`md`), deck_reset_progress_widget.dart:139-145 (`md`,… **Target đã đổi:** Replace all five `Text(...danger)` blocks with `MxFeedbackBand(title: context.l10n.deckWriteErrorTitle, message: context.deckWriteFailure(<failure>))`, used as-is with no API change — and set the prec |
| `SC-C3-20` | **DESIGN_SYSTEM_BLOCKED** | The duplication reproduces. route_not_found_screen.dart:35-46 wraps the whole body in `Semantics(namesRoute: true, label: context.l10n.pageNotFoundTitle)` around an `MxErrorState` whose `title:` at :39 is the same string, rendered… **Hợp đồng:** 6 — public contract of lib/shared/widgets/ (MxErrorState would have to annotate its own title node or gain a parameter s |
| `SC-C3-21` | **FIXED** | Reproduces structurally. trash_restore_target_sheet_widget.dart:74-86 wraps only the error face in `Column(mainAxisSize: MainAxisSize.min)`, and :67-73 spells out the mechanism ("the sheet hands down a bounded height and MxErrorSt… |
| `SC-C3-22` | **FIXED** | Reproduces at the cited lines. trash_restore_target_sheet_widget.dart:185-196 builds `MxListTile(title:…, subtitle:…, isSelected: target.deckId == selectedDeckId, onTap:…)` with no `leading`. mx_list_tile.dart:94-97 names the pair… |
| `SC-C3-23` | **FIXED** | Reproduces. trash_restore_target_sheet_widget.dart:78 passes `title: l10n.trashRestoreTargetTitle`, which app_en.arb:4120 defines as "Restore to" — the sheet's own name as the headline of a failed read. The same feature's screen d… |
| `SC-C3-24` | **REVISED_AND_FIXED** | The disagreement is real. reminder_settings_section_widget.dart:122 is `onTap: () => onTimePressed(settings.time)` and reminder_settings_screen.dart:138 forwards it unchanged to `showReminderTimePicker(context, current)`, while `_… **Target đã đổi:** Compute the pending value once in `ReminderSettingsScreen` under the finding's own gate — `ref.read(reminderTimeControllerProvider).rejection != null ? (draft ?? overview.settings.time) : overview.set |
| `SC-C3-25` | **REFUTED** | The observation is accurate — reminder_time_picker_widget.dart:24-28 passes `helpText` only, so `showTimePicker` falls back to `MaterialLocalizations.okButtonLabel` / `cancelButtonLabel` — but a legitimate documented mechanism exp… |
| `SC-C3-26` | **FIXED** | Reproduces, and it is the same defect as SC-C3-14 on the same lines. study_entry_screen.dart:132-135 is `MxErrorState(title: context.l10n.appTitle, message: context.l10n.studyNothingDueMessage)` with no retryLabel and no onRetry; … |
| `SC-C3-27` | **FIXED** | Reproduces. study_mode_chooser_widget.dart:74-85 builds `MxListTile(title:…, subtitle: isAvailable ? studyModeCardCount(count) : studyModeUnavailable(mode), onTap: isAvailable ? … : null)` and leaves `isEnabled` at its default `tr… |
