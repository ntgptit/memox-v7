...
| C10 | `MxErrorState` | `mx_error_state.dart` | [x] | **divergence** — repo wins: retry is primary, and only Flutter has `isRetrying`. The kit's error state takes no busy flag because the web's retry is a form submit the browser already narrates; in Flutter `ref.invalidate` is a refresh that `MxAsyncView` renders as no change at all, so without it a tap produced nothing on screen (M99.23, stage 1 of the #301–#310 integration). The design's note is prose, and M4.10n changed it so `MxErrorState` and `MxEmptyState` stop looking like two components |
