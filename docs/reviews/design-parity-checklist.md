...
| D14 | `ProfileScreen.jsx` | `settings_screen.dart` | [x] | **divergence** — Settings là màn thật từ M99.28 (stage 5 của đợt tích hợp #301–#310), không còn preview harness. Kit web có `ProfileScreen` cho một app có auth; màn này không có auth (AD-03) nên nó là **ba nhóm tuỳ chọn**: mặc định học toàn cục, theme, ngôn ngữ. Không có gì để mang sang từ kit cho tới khi profile tồn tại. Dùng chung: `MxCard`, `MxContentShell`, `MxConfirmDialog` (C-series), `MxTextField`, `RadioListTile` qua `app_radio_theme`. Phân kỳ có chủ ý ghi ở `docs/wbs.md` M99.28 (S9a: radio thay pill cho ba lựa chọn loại trừ nhau) |
