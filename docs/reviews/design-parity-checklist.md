...
| D22 | `settings_reminder_entry_section_widget.dart` | `settings_screen.dart` | [x] | **resolved** — M99.29: nhánh Settings có hàng vào `/settings/reminders` (wireframe M6 W1). Hàng **chỉ nhãn + chevron, không hiện trạng thái**: hiện `Off`/giờ ở đây buộc `features/settings/presentation/` theo dõi state của `features/reminder/` — đúng cross-feature import mà M6 R1 và guard chặn. Đặt sau nhóm Ngôn ngữ và trước Reset: Reset là hành động phá huỷ nên luôn đứng cuối. Trước pass này route tồn tại, screen render, deep link chạy, và **không màn nào trong app link tới nó** — mọi gate đều xanh vì không gate nào hỏi "có ai gọi route này không"; `test/app/router/settings_reminder_entry_test.dart` là cái hỏi |
