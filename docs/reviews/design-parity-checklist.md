...
| D27 | `card_detail_screen.dart`, `card_detail_content_widget.dart`, `card_detail_state_widget.dart`, `card_history_section_widget.dart` | `progress_*_widget.dart`, `settings_*_section_widget.dart`, `study_home_body_section_widget.dart` | [x] | **resolved** — M99.60, **phân kỳ ghi ở M99.31 đã bị chính chủ dự án đảo lại (2026-08-26)**. Lập luận cũ — "một trang đọc về đúng một thẻ, chia thành ba hộp thì ba band đọc thành ba đối tượng" — đúng về ý định và sai về kết quả đo được: không bọc gì thì cả màn là một dải chữ phẳng, cùng nền, cùng cỡ, phân cách bằng khoảng trắng, và không có gì nói *phần nào* là nội dung của thẻ. Hình dạng mới: hero panel bọc `MxCard` viền `borderAccent` (bề mặt duy nhất của màn được nâng, `AppElevation.card`), bảng số liệu SRS là khối `surfaceMuted` bo `AppRadius.md`, timeline bọc `MxCard` **phẳng** viền `borderSubtle` theo đúng D20. Section header rời khỏi `titleSmall` sang `sectionLabel` chữ hoa, nằm **ngoài** card nó gọi tên. **Không đổi field, nút, dữ liệu hay hành vi nào** — chỉ bố cục và màu. Nợ chung của hàng cũ ("màn đọc thứ hai phải theo hàng này") **đóng lại**: màn đọc thứ hai theo D20 như mọi cột cuộn khác, với ngoại lệ duy nhất là một hero panel được nâng cho mỗi màn |
