...
| **Updated by task** | M99.23 — Trash: bảng `delete_batches`, cột `delete_batch_id` trên `decks`/`cards`, `end_reason = content_deleted`, bất biến 31…35, và bất biến 1…5/15/29 đo trên hàng đang active |
...
| `delete_batch_id` | TEXT NULL | NULL = deck đang active. Khác NULL = tombstone thuộc batch đó (BR-182, BR-184). → `delete_batches(id)` ON DELETE CASCADE |
...
| `delete_batch_id` | TEXT NULL | NULL = card đang active. Khác NULL = tombstone thuộc batch đó (BR-182, BR-184). → `delete_batches(id)` ON DELETE CASCADE |
| `created_at` | DATETIME NOT NULL | UTC |
| `updated_at` | DATETIME NOT NULL | UTC |
...
| `end_reason` | TEXT NULL | `user_exit` \| `scheduler_reset` \| `stale_generation` \| `persistence_error` \| `interrupted` \| `content_deleted` (BR-80, BR-185). NULL khi `in_progress` hoặc `completed` |
...
| `invalidated` | `content_deleted` | deck hoặc card mà phiên đang chạy trên đó bị chuyển vào Trash (BR-185) |
...
Một hàng cho mỗi **lần xoá** của người dùng (BR-182). Hàng của `decks`/`cards`
không bị chép đi đâu cả — chúng chỉ nhận `delete_batch_id`.

| Cột | Kiểu | Ghi chú |
|---|---|---|
| `id` | TEXT PK | UUID sinh phía client (AD-03) |
| `item_type` | TEXT NOT NULL | `'card'` \| `'deck'` — loại của **item root**, tức thứ người dùng đã chạm (BR-192) |
| `root_item_id` | TEXT NOT NULL | id của card hoặc deck đó. Không phải FK: hai bảng đích, và cascade đã đi theo chiều ngược lại |
| `deleted_at` | DATETIME NOT NULL | UTC. Mốc duy nhất của retention 30 ngày (BR-190) |
| `owner_id` | TEXT NULL | NULL = local profile (AD-03) |
...
và `card_tags` lo phần còn lại (BR-191). Điều kiện tiên quyết của BR-191 —
...
invariant 2 báo vi phạm cho mọi deck vừa được dọn rỗng đúng BR-186, và invariant
...
-- 29. Sub-deck đã rỗng nhưng vẫn mang type (BR-163, BR-186)
...
--     restore mới là nơi độ sâu của nó được thẩm định lại (BR-187).
...
-- 31. Card đang active nằm trong một deck đã xoá (BR-182, BR-184)
...
-- 32. Deck đang active nằm dưới một deck đã xoá (BR-182, BR-184)
SELECT d.id FROM decks d
JOIN decks p ON p.id = d.parent_deck_id
WHERE d.delete_batch_id IS NULL AND p.delete_batch_id IS NOT NULL;

-- 33. Batch không còn hàng nào (BR-191)
...
--     xuyên qua một batch mà điều kiện tiên quyết của BR-191 lẽ ra phải chặn.
...
-- 34. Tombstone bị xoá SAU tổ tiên đã xoá của nó (BR-184)
...
--     "batch eligible thì mọi descendant của nó cũng eligible" đúng, và BR-191
--     dựa vào điều đó.
SELECT d.id FROM decks d
JOIN decks p ON p.id = d.parent_deck_id
...
-- 35. Batch không trỏ về một item root mang chính batch đó (BR-182)
SELECT b.id FROM delete_batches b
...
| 6 | Data-only: sub-deck rỗng nhưng còn mang type về `'unset'` — chính vị từ của bất biến 29 dùng làm `UPDATE` (M99.15, BR-163) |
| 7 | Data-only: điền `decks.first_answered_at` bằng `MIN(learned_at)` của cây cho root đã học xong ít nhất một thẻ (M99.16, BR-13) |
| 8 | Trash: bảng `delete_batches`; cột `decks.delete_batch_id`, `cards.delete_batch_id` + ba index; nới `CHECK` của `study_sessions.end_reason` để nhận `content_deleted` (M99.23, BR-182…BR-193) |
