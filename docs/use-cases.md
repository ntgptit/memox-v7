...
| **Updated by task** | M99.23 — UC-12: Trash và khôi phục item đã xoá (soft-delete → Trash → restore có target / purge) |
...
## UC-12 · Trash và khôi phục item đã xoá
...
dùng biết nó tồn tại (BR-183 chỉ nói cái gì bị ẩn khỏi *bề mặt active*)
...
1. Người dùng xoá một card hoặc một deck từ luồng đã có (UC-04, UC-08). Hệ thống
   ...
   (BR-182, BR-184, BR-185, BR-186).
2. Màn đang đứng báo item đã chuyển vào Trash và hiện Undo trong một khoảng thời
   gian có hạn (BR-182). Mọi bề mặt active cập nhật ngay — item biến mất khỏi
   danh sách, khỏi đếm, khỏi search và khỏi study (BR-183).
...
5. Người dùng chọn `Restore` trên một hàng. Hệ thống mở picker target dựng từ
   **đúng** eligibility của move: chỉ deck đang active thoả loại nội dung, độ
   sâu và scheduler/generation mới xuất hiện (BR-187).
...
- **A2 — Chọn nhiều:** người dùng bật chế độ chọn trong Trash. Thanh hành động
  ...
  mọi hàng deck không chọn được và ngược lại; UI nói rõ vì sao (BR-192).
- **A3 — Purge vĩnh viễn:** người dùng chọn `Delete permanently`. Hộp thoại nêu
  ...
  động an toàn, và chỉ hành động phá huỷ mang vai trò màu destructive (BR-192).
  Xác nhận chạy một transaction xoá cứng batch và cascade (BR-191).
...
- **E1 — Không có target hợp lệ:** picker mở ra rỗng và giải thích vì sao (cây
  ...
  MUST NOT hiện hàng bị vô hiệu hoá trông như chọn được (BR-187).
- **E2 — Target hết hợp lệ giữa chừng:** cây đổi sau khi picker mở. Transaction
  từ chối bằng lý do có kiểu, không ghi gì, và picker tải lại danh sách (BR-187).
...
**Business rules:** BR-182…BR-193, và BR-55, BR-63, BR-64, BR-70, BR-71, BR-74,
