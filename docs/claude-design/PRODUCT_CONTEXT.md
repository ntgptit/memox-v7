...
| U7 | Several repository documents are behind the code (product scope still calling Settings a placeholder; a flow document calling Study unbuilt; the session end-reason rule listing five values where seven exist; an old state table locking the algorithm at the first review instead of the first learned card). This handoff follows the code and the newest rules | Resolved here in favour of code |
