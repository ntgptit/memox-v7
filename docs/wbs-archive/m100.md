...
- **Nhánh / PR:** `claude/tokyo-redesign-phase-4` · #562
- **Scope:**
...
- **Audit (plan §4.4):**
  ...
  - UI/UX, sweep 3 tại `e2398cf8`:
    ...
    - Các call site còn lại đã được xác nhận có pin: MxTextField × 7,
      ...
      progress range.
  - UI/UX, sweep 4 tại `b8dc7cc4` (độ phủ regression pin): không có
    P0/P1/P2, không có divergence chưa duyệt. Reviewer chạy lại test sửa
    sweep 3: 4/4. 88 PNG đổi được map về từng màn. Mọi call site phase chạm
    tới đều được đếm bằng grep và đều có pin: `MxTextField` × 7,
    `MxSearchField` × 3, `MxPillButton` × 4, `MxSwitchRow` × 3; không có
    `ChoiceChip`/`FilterChip` thô.
    - P3 (trace): `MxTextField` thêm tag trong card editor
      (`card_tag_section_widget.dart`) được pin bằng interaction
      (`card_tag_after_error_test.dart`) và rect của section, chưa có
      geometry riêng. Mức phủ này giống các call site cùng loại, không phải
      regression.
  - UI/UX, sweep 5 tại `b8dc7cc4` (so từng giá trị với handoff): không có
    P0/P1/P2, không có divergence chưa duyệt. Đã so TextField, SearchField,
    Chip, Switch, Slider và SegmentedControl ở light, dark, HC, reduced motion
    và focus bàn phím. Mỗi control chỉ có một cơ chế focus: field dùng viền,
    control khác dùng `MxFocusRing`.
    - P3 (trace): comment `side:` trong `app_chip_theme.dart` nói focus nằm ở
      `_fillFor`, nhưng `_fillFor` không có nhánh `focused`. Focus thật do
      `MxFocusRing` và wash mặc định của InkWell đảm nhận. Comment này cũ từ
      trước Phase 4, không phải lỗi hành vi.
  - Clean stop: sweep 4 và sweep 5 chạy độc lập trên cùng HEAD, không có fix
    nào xen giữa, nên là hai sweep sạch liên tiếp. Audit architecture/logic
    đã sạch từ pass 1.
...
- **Acceptance criteria:**
  - [x] Text field: fill, viền, 52/40, hàng lỗi, disabled 0.38 có test ở tầng
    theme và widget.
  - [x] Search field nghỉ/active và slot voice; chip chưa chọn `surfaceContainer`.
  - [x] `MxSwitch` 44×26, thumb 20 inset 3, một node toggled trên hàng.
  - [x] `MxSlider` và `MxSegmentedControl` có test và Widgetbook.
  - [x] `flutter analyze` 0/0 repo-wide; full host suite green; guard 0 findings
  - [x] Goldens re-authored on Linux, `TZ=UTC`; gallery republished at the pinned URL
...
- **Goal:** Bề mặt và hàng theo handoff: card 20, MxIconTile, overline caption,
  ...
  `docs/superpowers/plans/2026-09-13-tokyo-handoff-redesign.md`.
- **Nhánh / PR:** `claude/tokyo-redesign-phase-3` · #561
- **Scope:**
...
- **Audit notes:**
  ...
  - Xác nhận architecture/logic trên delta `6a327b83..f9efe7da` (subagent mới):
    sạch, không có P0–P3. Comment đã khớp token (`captionSize` 12, `labelSmall`
    w600, `sectionLabelTracking` 1.2). Lập luận high contrast đúng khi đọc cách
    dựng theme; assertion so hai theme khác nhau thật. Ba file test 39/39, không
    `skip`, không nới kỳ vọng. `lib/` chỉ đổi một dòng comment.
  - UI/UX sweep 3 (`f9efe7da`, subagent mới): không có P0–P2, không có
    divergence chưa duyệt. Không PNG nào đổi sau `a00e0270`. Reviewer tự chạy
    lại: high contrast và floors 30/30, search 18/18, deck 9/9. Đã xem 8 golden.
    Vòng "Deck progress" trên card list là `_ProgressRing` có sẵn (`success`),
    không phải `MxMasteryRing`.
  - UI/UX sweep 4 (`f9efe7da`, subagent mới): không có P0–P2, không có
    divergence chưa duyệt — sweep sạch thứ hai liên tiếp, đạt visual clean stop.
    Đã xem golden dark và dark high contrast của các bề mặt Phase 3 cùng
    `tag_catalog_320_x2`: không cắt chữ, không tràn. Bốn widget mới đã đăng ký
    trong Widgetbook. Semantics một node cho mỗi control (ring, badge). Focus
    của hàng deck và hàng search đi qua `MxFocusRing` của `MxPressable`, không
    có `InkWell` trần.
- **Editable documents:** `docs/wbs.md`,
  `docs/design-system/tokyo-component-mapping.md`,
...
- **Acceptance criteria:**
  - [x] Card-surface recipe bo 20 và pad 20; `tile` giữ 12.
  - [x] `MxIconTile`, `MxRowGroup`, `MxStatusBadge`, `MxMasteryRing` có test và
    Widgetbook.
  - [x] Hàng deck của Library: tile · tên + workload · ring · overflow; vẫn học
    được một deck từ tab Study; ring `mastery` chỉ khi `isFullyLearned` (BR-88).
  - [x] `flutter analyze` 0/0 repo-wide; full host suite green; guard 0 findings
  - [x] `integration_test/` trên emulator xanh.
  - [x] Goldens re-authored on Linux, `TZ=UTC`; gallery republished at the pinned URL
...
- **Goal:** Nút theo handoff: compact 36, tonal cho hành động phụ tiến tới, icon
  ...
  `docs/superpowers/plans/2026-09-13-tokyo-handoff-redesign.md`.
- **Nhánh / PR:** `claude/tokyo-redesign-phase-2` · #560
- **Scope:**
...
- **Audit notes:**
  ...
  - UI P3 (giữ, không chặn): `buildOutlinedIconButtonStyle` chỉ đặt
    `minimumSize` 36. Glyph `content` 20 cộng padding 8+8 vừa đúng 36, nhưng một
    glyph lớn hơn trên biến thể outlined sẽ làm vòng phình quá 36 mà không gate
    nào chặn. Xem lại khi có call site outlined với glyph > 20.
  - UI P3 (giữ, không chặn): FAB extended đè góc thẻ sub-deck thứ ba ở
    `deck_list_level`. Chủ dự án chủ động hoãn; plan Task 15 là nơi xử lý lại
    việc hiện đủ ba hàng deck (PLAN-DEV-2.9).
- **Editable documents:** `docs/wbs.md`,
  `docs/design-system/tokyo-component-mapping.md`,
  `docs/superpowers/plans/2026-09-13-tokyo-handoff-redesign.md` (ghi deviation).
...
- **Acceptance criteria:**
  - [x] `MxActionButtonSize.compact` vẽ 36, đích chạm 48; loading giữ bề rộng.
  - [x] Tám call site hành động phụ tiến tới dùng `MxActionButtonVariant.tonal`.
  - [x] `MxIconButton` vẽ vòng 36, glyph 20 (bar 24), tint `primary` 8% / 14%.
  - [x] `MxFab` là FAB extended có nhãn, cao 52, `primary`, `shadow-fab`.
  - [x] `flutter analyze` 0/0 repo-wide; full host suite green; guard 0 findings
  - [x] `integration_test/` trên emulator xanh.
  - [x] Goldens re-authored on Linux, `TZ=UTC`; gallery republished at the pinned URL
...
- **Goal:** Nền tảng theo handoff: một family Plus Jakarta Sans với bảy vai chữ,
  ...
  cuộn 48 — Phase 1 của
  `docs/superpowers/plans/2026-09-13-tokyo-handoff-redesign.md`.
- **Nhánh / PR:** `claude/tokyo-redesign-phase-1` · #559
- **Scope:**
...
- **Acceptance criteria:**
  - [x] 15 slot `TextTheme` khớp bảng D1, một family; weight registry đúng
    {400, 500, 600, 700, 800}, mọi nguồn 700/800 có tên.
  - [x] `AppRadius`, `AppIconSize`, `MxIconSize`, `AppSpacing` theo thang
    handoff; `AppStateOpacity.stateLayerPressed` = 0.12.
  - [x] `mxScreenGutter` = 16 ở mọi bề rộng; `mxScrollEndInsetOf` = 48 khi
...
- **Audit:** hai audit độc lập — architecture/logic và UI/UX — cùng đạt
  CLEAN_STOP trên `b8a31e68`, không P0–P2. Ba P3 docs sửa ngay trong phase:
  `chip-spec.md` §2.5 trích câu SHOULD cũ của §3c như luật hiện hành
  (architecture); header `v1-freeze.md` còn ghi `M100.85` (UI); D12 bỏ sót vế
  foundations "24 bottom sheet top corners" (UI). Sau fix, coordinator kiểm
  bằng `check_docs.py`; không mở vòng audit thứ ba vì cả hai đã clean stop và
  không fix nào đổi kết luận.
- **Editable documents:** `docs/wbs.md`, `docs/wbs-archive/m100.md`,
  `docs/design-system/v1-freeze.md`,
  `docs/design-system/tokyo-component-mapping.md`,
  `docs/design-system/chip-spec.md` (audit P3),
  `docs/superpowers/plans/2026-09-13-tokyo-handoff-redesign.md` (D12).
- **Output:** `docs/design-system/handoff/memox-flutter-handoff.json` (SHA-256
  ...
  như trên.
- **Acceptance criteria:**
  - [x] Hash của bản trong repo bằng hash của bản chủ dự án giao.
  - [x] §9 chép nguyên văn owner decisions 1–10 và D1–D27 từ plan, giữ số và ID.
  - [x] `check_docs.py` và prompt contract xanh; không đổi code.
  - [x] Hai audit độc lập (architecture/logic, UI/UX) đạt clean stop.
...
### M100.87 · Palette, mực chữ và độ sâu theo handoff Tokyo

- **Status:** done (2026-09-13)
- **Owner:** Claude
- **Goal:** Thay giá trị của cả 45 role `ColorScheme`, token semantic, border và
  shadow bằng giá trị của handoff Tokyo — bước foundations đầu tiên của đợt
  redesign mở ở M100.85.
- **Nhánh / PR:** `claude/tokyo-foundations`
- **Số task:** M100.87. Nhánh này làm dưới số M100.86, nhưng PR #555
  (`claude/chip-flutter-design-spec-fd5cea`) đã nhận M100.86 công khai trước,
  nên đổi số lúc push (2026-09-13); mọi chỗ ghi số trong code và docs của nhánh
  đổi theo.
- **Đồng bộ với main:** #555 merge trước (Chip → `primaryContainer`), nên
  nhánh này merge `origin/main` trước khi merge. 39 golden cả hai PR cùng đổi
  được author lại trên code đã gộp — chip của #555 vẽ trên palette Tokyo — chứ
  không chọn một bên.
- **Scope:** `lib/core/theme/foundations/` (role, surface, border, semantic,
  elevation); `AppInk`; các slot theme vẽ brand thành chữ; test và guard canh
  các hợp đồng đó, dời theo giá trị mới; golden.
- **Out of scope:** typography (M100.88), spacing (M100.89), mọi spec component
  của section A–G.
- **Quyết định của chủ dự án (2026-09-13):**
  - Hex handoff dùng **nguyên văn cho fill**. Chữ dùng mực riêng cùng hue, đậm
    hơn — `accentInk`, `successInk`, `warningInk`, `dangerInk`, `infoInk`,
    `secondaryInk`, `tertiaryInk`, `inversePrimaryInk` trên
    `AppSemanticColors` — giải theo L\* để đạt 4.6:1 trên page, card và tile.
    `AppInk` chỉ resolve ra mực; mực của dark là chính fill.
  - **Kit thắng binding canonical M3.** Label của TextButton, OutlinedButton,
    TabBar đọc `accentInk` (`RoleBinding.requiresSemantic` mới trong guard AST);
    ListTile `selectedColor` → `onPrimaryContainer`; Card →
    `surfaceContainerLowest`, `MxCard.recessed` → `surface`;
    `surfaceEmphasis` = `secondaryContainer`; fill của `MxProgressBar` =
    `primary` (tone `primary / mastery` của LinearProgress).
  - **Giữ hex kit, chấp nhận dưới ngưỡng**, mỗi chỗ ghim bằng con số đo làm sàn,
    không gỡ gate: `outline` trên `surfaceContainerHigh` light 2.9, trên
    `surfaceContainer`/`High` dark 2.6/2.2; fill `success`/`warning` light trên
    page 2.8/2.0; thumb switch nghỉ 2.7 light / 1.9 dark; label FilledButton khi
    nhấn ở light 3.9 (brand) / 4.1 (destructive). Visual audit ghi các cặp
    control edge đó qua `NonTextContrastRule.acceptedFloors`: sàn chỉ đúng
    cho cặp (border, nền) đã chấp nhận, nền khác vẫn đòi 3:1.
  - **Shadow:** card = `shadow-soft` `0 1px 2px` @4%, raised = `shadow-card`
    `0 12px 32px` @10%, overlay = `shadow-fab` `0 8px 24px` @12%; dark = rim
    hairline cộng drop của kit. Sàn lift của card light hạ 6.0 → 5.0 (đo 5.24).
- **Nợ ghi lại:** ở dark, knob switch disabled (2.83:1) nổi hơn thumb đang sống
  (1.96:1). Ghim ở `app_toggle_themes_test.dart`; sửa trong PR spec Switch
  (disabled = cả control @38%), không phải bằng palette.
- **Editable documents:** `docs/wbs.md`, `docs/wbs-archive/m100.md`,
  `docs/design-system/theme-architecture.md`,
  `docs/design-system/tokyo-component-mapping.md`,
  `docs/design-system/ad-14-color-and-depth.md`.
- **Output:** palette mới trong `foundations/`; tám field mực trên
  `AppSemanticColors`; 26 test TOKYO-2 bật lại; golden author lại trên Linux.
- **Acceptance criteria:**
  - [x] `flutter analyze` toàn repo sạch; `code-verification-guard-v2` sạch.
  - [x] Full host suite (không golden) xanh.
  - [x] Golden author lại trên Linux (WSL); gallery publish lại ở URL ghim.
  - [x] Không test nào bị xoá, `skip` hay `exclude`; mỗi ngoại lệ dưới ngưỡng
        mang con số đo của nó.
- **Dependencies:** M100.85
- **Tests required:** `app_theme_test`, `app_palette_test`,
  `color_scheme_roles_test`, `color_system_rules_test`,
  `control_border_grounds_test`, `app_elevation_test`,
  `m3_role_binding_guard_test`, `m3_role_contract_test`,
  `high_contrast_figures_test` và các test contrast feature đã dời.
- **Checklist phases:** phase 6 — theme/token.

---

### M100.86 · Chip đổi sang primaryContainer — thực hiện handoff, lệch kit-vs-canonical hỏi thẳng chủ dự án

- **Status:** done (2026-09-13)
- **Owner:** Claude
- **Goal:** Triển khai đặc tả component Chip của handoff Tokyo (mục D · Inputs
  & forms) dưới quyền chung ở M100.85 (`v1-freeze.md` §3c, điều kiện 2). Spec
  đặt tên role trực tiếp cho selected fill/label:
  `primaryContainer`/`onPrimaryContainer`, lệch với default canonical của
  `_ChoiceChipDefaultsM3` (`secondaryContainer`) mà `m3_role_bindings.dart`
  đang ghim. §3c nói rõ chỗ kit mâu thuẫn M3 canonical SHOULD hỏi chủ dự án
  từng ca thay vì tự chọn — hỏi thẳng ba hướng (giữ nguyên / đổi chỉ Chip /
  đổi cả Chip+SegmentedButton+NavigationBar); chủ dự án chọn **chỉ Chip**,
  chấp nhận hệ quả: ba component "đang active" không còn cùng ngôn ngữ màu.
- **Nhánh / PR:** `claude/chip-flutter-design-spec-fd5cea`
- **Scope:** `app_chip_theme.dart` (`_restingFill`/`_labelColorFor` selected →
  `primaryContainer`/`onPrimaryContainer`; icon `AppIconSize.sm`→`.md`),
  `mx_pill_button.dart` (icon theo), `m3_role_bindings.dart` (đảo
  requires/refuses hai binding ChoiceChip), `m3_role_contract_test.dart`,
  `m3_combined_state_test.dart`, `mx_pill_button_theme_test.dart`,
  `mx_pill_button_test.dart` (comment), `docs/design-system/chip-spec.md`
  (mới). `SegmentedButton`/`NavigationBar` **không đụng** — đã kiểm bằng grep,
  không có hunk nào trong hai file theme của chúng; đây là phạm vi của riêng
  task Chip, không phải hợp đồng đóng băng chặn lại.
- **Editable documents:** `docs/design-system/tokyo-component-mapping.md`,
  `docs/design-system/chip-spec.md`, `docs/wbs-archive/m100.md`.
- **Output:** một role binding đổi theo handoff; 39 golden ảnh hưởng thật
  (không phải drive-by).
- **Acceptance criteria:** 154/154 test xanh trên bộ file bị chạm trực tiếp
  (`m3_role_contract_test`, `m3_combined_state_test`,
  `m3_role_binding_guard_test`, bốn file `mx_pill_button_*`);
  `dod_check.sh --changed --base origin/main` xanh (gồm
  `mx_stress_test.dart`'s tap-target sweep 2.0x text scale qua mọi shared
  component); golden Linux (WSL) — 343 sub-test xanh, **đúng 39 PNG đổi thật**
  trên 44 file golden-tagged, còn lại byte-khớp committed; gallery
  `test/demo/` regenerate + republish đúng URL ghim.
- **Dependencies:** M100.36 (chip đóng hợp đồng lần đầu), M100.85 (V1 mở lại
  cho redesign — điều kiện quyền hạn của task này).
- **Tests required:** không test mới — bảy assertion trong bốn file theme
  contract đổi giá trị mong đợi theo role mới; đáng nói nhất là
  `m3_combined_state_test.dart`'s "disabled does not leak a live role":
  `isNot(scheme.secondaryContainer)` sẽ **vẫn xanh** nếu để nguyên (chưa từng
  đúng giá trị đó), nhưng sẽ ngừng kiểm tra bất cứ điều gì có ý nghĩa cho
  component này — đổi thành `isNot(scheme.primaryContainer)` để giữ đúng bất
  biến đang kiểm.
- **Phạm vi ảnh hưởng rộng hơn dự tính ban đầu, và đã đo chứ không giả định.**
  `ChipThemeData.iconTheme` là fall-through chung cho **cả họ Chip**, không
  riêng `MxPillButton` — thay đổi kích thước icon kéo theo tag editor's
  `Chip(onDeleted:)` và `ActionChip` (+ Add tag) trong
  `card_tag_section_widget.dart`, hai call site không nằm trong đặc tả gốc.
  Lần đầu chạy `--update-goldens` cho batch đầy đủ báo **115+ file đổi**, gồm
  cả `typography`/`scaffold`/`text_button` — hoàn toàn không liên quan. Đối
  chiếu bằng `compare` (không `update`) trên `origin/main` sạch xác nhận
  committed goldens **không** stale (86/86 xanh); một lượt chạy sạch hoàn
  toàn (`flutter clean` + rebuild, không tái dùng cache giữa hai lần chuyển
  nhánh) trên đúng nhánh Chip cho đúng 4 file pill đổi ở một file test đơn lẻ,
  và đúng 39 file trên toàn batch — khớp chính xác với những màn thật sự vẽ
  một Chip-family widget. Nguyên nhân của lượt đo sai: process mồ côi từ các
  lần `flutter test` bị timeout/kill giữa chừng vẫn sống trong WSL2 VM (kill
  ở phía `wsl.exe` không lan tới process bên trong VM), tranh chấp
  `.dart_tool`/build cache với lượt chạy sau.
- **Checklist phases:** phase 6 — shared components / design system.

---

### M100.85 · V1 mở lại cho redesign theo handoff Tokyo

- **Status:** done (2026-09-13)
- **Owner:** Claude
- **Goal:** Ghi vào repo quyết định của chủ dự án rằng Design System V1 được mở
  lại cho một đợt redesign theo handoff Tokyo, để mọi phiên — kể cả subagent và
  session cloud, vốn đều được nạp `CLAUDE.md` — đọc cùng một quyết định, thay vì
  chỉ một phiên nhớ nó.
- **Scope:** reopen record `v1-freeze.md` §3c (điều kiện 2); đoạn freeze trong
  `CLAUDE.md`; sáu hàng `DESIGN_SYSTEM_BLOCKED` của `wbs.md` chuyển sang mở khoá.
- **Out of scope:** mọi thay đổi trong `lib/` và `test/` (redesign là các task
  sau); commit bản handoff vào repo; đóng băng lại V1.
- **Bối cảnh:** phiên đầu tiên đọc handoff như một loạt đối chiếu và mở ba PR
  docs-only (#551 FilledButton, #552 TonalButton, #553 FAB), cả ba kết luận
  "không sửa, đi qua §3". Chủ dự án bác cách đọc đó: đây là redesign, không phải
  sửa lỗi. Ba PR bị đóng, không merge.
- **Editable documents:** `docs/design-system/v1-freeze.md`, `CLAUDE.md`,
  `docs/wbs.md`, `docs/wbs-archive/m100.md`.
- **Output:** `v1-freeze.md` §3c; mục "Design System V1 was frozen, and is
  reopened for the Tokyo redesign" trong `CLAUDE.md`.
- **Acceptance criteria:**
  - [x] `check_docs.py` xanh.
  - [x] Không file nào ngoài `Editable documents` bị sửa.
  - [x] §3c nói rõ cái gì mở và cái gì không: dòng 14, AD-23, và test canh hợp
        đồng phải được dời theo giá trị mới chứ không bị gỡ.
- **Dependencies:** M100.73, M100.76
- **Tests required:** none — document only. Không test nào ghim văn bản của
  `CLAUDE.md` hay `v1-freeze.md` (đã grep).
- **Checklist phases:** phase 6 — theme/token.
