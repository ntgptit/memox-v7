...
- **Quyết định của chủ dự án (2026-09-13):**
  - Hex handoff dùng **nguyên văn cho fill**. Chữ dùng mực riêng cùng hue, đậm
    ...
    `inversePrimaryInk` trên `AppSemanticColors` — giải theo L\* để đạt 4.6:1
    trên page, card và tile. `AppInk` chỉ resolve ra mực.
  ...
  - **Giữ hex kit, chấp nhận dưới ngưỡng**, mỗi chỗ ghim bằng con số đo làm sàn,
    ...
    nhấn ở light 3.9 (brand) / 4.1 (destructive).
...
- **Output:** palette mới trong `foundations/`; sáu field mực trên
