...
| **Updated by task** | M100.16 — BR-269: promote sub-deck thành root là thao tác riêng, chọn scheduler tường minh, reset state/session nhưng giữ history; M100.10 — BR-268: thứ tự manual, sibling-only, atomic và không đụng dữ liệu cây/học |
...
Trạng thái hiện tại: **BR-01…BR-269**, không trùng, không thiếu.
...
| BR-269 | active | Promote sub-deck thành root MUST là thao tác riêng, không phải Move với parent NULL. Người dùng MUST chọn scheduler hợp lệ; source MUST là sub-deck không có direct card. Trong đúng một transaction, source trở thành root `deck`, scheduler version/generation mới là 1, `first_answered_at` và `study_config` NULL, toàn bộ subtree nhận `root_deck_id` mới và study state được seed lại; session in-progress có `deck_id` hoặc queue card chạm subtree MUST thành `invalidated/subtree_promoted`. Card, cây descendant và `study_answers` MUST được giữ; transaction lỗi MUST rollback toàn bộ. UI MUST xác nhận và nêu rõ state/session bị reset nhưng history được giữ. | repository + database + UI | UC-23, AD-06, AD-09, AD-10 |
